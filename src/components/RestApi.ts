...
export const RestApi = ({
    ...
    const gatewayDomainName = new aws.apigateway.DomainName(`${nameTag}_api_domain_name`, {
        domainName,
    ...
    const basepathMapping = new aws.apigateway.BasePathMapping(`${nameTag}_api_base_path_mapping`, {
        restApi: api.api.id,
        stageName: pulumi.getStack() === 'prod' ? 'prod' : 'dev',
        domainName: domainName
    })
    // Create Route53 Record
    const zone = aws.route53.getZoneOutput({name: new pulumi.Config().require("rootDomain")})
